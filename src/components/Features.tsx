...
const Features = () => {
  return <section className="relative py-20 px-4 md:px-8 bg-gradient-to-br from-background via-ai-surface/20 to-background overflow-hidden">
      {/* Background Pattern */}
      <div className="absolute inset-0 opacity-20 bg-zinc-200">
      ...
      <div className="relative max-w-7xl mx-auto">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
          {features.map((feature, index) => <AnimatedCard key={index} hoverEffect="tilt" delay={index * 0.1} className="h-full group">
              <motion.div className="p-8 h-full flex flex-col bg-gradient-to-br from-background to-ai-surface/30 border-2 border-ai-border backdrop-blur-sm" whileHover={{
            background: "linear-gradient(135deg, hsl(var(--background)), hsl(var(--ai-surface)))"
          }} transition={{
            duration: 0.3
          }}>
                {/* Animated Icon Container */}
                <motion.div className="relative h-16 w-16 rounded-2xl bg-gradient-to-br from-ai-primary/20 to-ai-secondary/20 flex items-center justify-center mb-6 border border-ai-border" whileHover={{
              scale: 1.1,
              rotate: [0, -5, 5, 0],
              ...
              stiffness: 300
                ...
                duration: 0.2
                  ...
                  {/* Glow Effect */}
                  <motion.div className="absolute inset-0 rounded-2xl bg-gradient-to-br from-ai-primary to-ai-secondary opacity-0 blur-xl" whileHover={{
                opacity: 0.3
              }} transition={{
                duration: 0.3
              }} />
                ...
                {/* Content */}
                <motion.h3 className="text-xl font-bold mb-3 text-foreground group-hover:text-ai-primary transition-colors" initial={{
              opacity: 0.8
            }} whileHover={{
              opacity: 1
            }}>
                ...
                <motion.p className="text-muted-foreground flex-grow leading-relaxed" initial={{
              opacity: 0.7
            }} whileHover={{
              opacity: 1
            }}>
                ...
                {/* Decorative Element */}
                <motion.div className="mt-6 h-1 w-0 bg-gradient-to-r from-ai-primary to-ai-secondary rounded-full" whileHover={{
              width: "100%"
            }} transition={{
              duration: 0.4,
              ease: "easeInOut"
